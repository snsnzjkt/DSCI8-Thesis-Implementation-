...
class SCSIDModel(nn.Module):
    ...
    Optimized architecture for high accuracy and low FPR:
    - Enhanced Fire modules with residual connections
    - Efficient ConvSeek blocks with attention
    ...
    def __init__(self, input_features: int = 42, num_classes: int = 15):
        ...
        # ==================== INPUT PROCESSING ====================
        # Efficient input processing with channel attention
        self.input_conv = nn.Conv1d(1, 16, kernel_size=3, padding=1)
        ...
        # ==================== FIRE MODULES ====================
        # Optimized Fire modules with residual connections
        self.fire1 = FireModule(
            input_channels=16,
            squeeze_channels=8,      # More aggressive squeeze (1:2 ratio)
            expand1x1_channels=16,   # Balanced expansion
            expand3x3_channels=16    # Equal expansion for both paths
        )  # Output: 32 channels
        
        self.fire2 = FireModule(
            input_channels=32,
            squeeze_channels=16,     # Maintain squeeze ratio
            expand1x1_channels=32,   # Consistent expansion
            expand3x3_channels=32    # Equal distribution
        )  # Output: 64 channels
...
def apply_quantization(model: SCSIDModel) -> nn.Module:
    """
    Apply simplified quantization for inference efficiency (thesis requirement: 75% size reduction)
    Uses a more stable approach to quantization that works across different platforms.
    ...
    try:
        ...
        # Simple weight quantization for each module
        for name, module in quantized_model.named_modules():
            if isinstance(module, (nn.Linear, nn.Conv1d, nn.Conv2d)):
                ...
                if module.bias is not None:
                    # Keep bias in fp32 for better accuracy
                    module.bias.data = module.bias.data.to(torch.float32)
        
        print("   ✓ Quantization complete")
        print("   Model size reduced")
        
        return quantized_model
        
    except Exception as e:
        print(f"   ⚠️ WARNING: Quantization failed: {e}")
        print("   Returning original unquantized model")
        return model
